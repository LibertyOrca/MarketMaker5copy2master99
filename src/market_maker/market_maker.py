...
class ExchangeInterface:
    ...
    def cancel_all_orders(self):
        ...
        orders = self.bitmex.http_open_orders(settings.SYMBOL)
    ...
    def amend_bulk_orders(self, orders, symbol):
        if self.dry_run:
            return orders
        return self.bitmex.amend_bulk_orders(orders, symbol = symbol)

    def create_bulk_orders(self, orders, symbol):
        if self.dry_run:
            return orders
        return self.bitmex.create_bulk_orders(orders, symbol = symbol)

#still returning the standard insturments. 
...
class OrderManager:
    ...
    def print_status(self):
        ...
        instrumentChoice = self.instrumentChoice()
        ...
        if position['currentQty'] != 0:
            ...
            logger.info("current price change var: %d" % priceChange)
            logger.info("Contracts Traded This Run: %d" % (self.running_qty - self.starting_qty))
            logger.info("Total Contract Delta: %.4f XBT" % (self.exchange.calc_delta())['spot'])
            for symbol in symbolList:
                logger.info(portfolio[symbol]["currentQty"])
            logger.info(settings.CONTRACTS)
            logger.info(instrumentChoice)

    def get_ticker(self):
        instrumentChoice = self.instrumentChoice()
        ticker = self.exchange.get_ticker(instrumentChoice)
    ...
    def priceChange(self):
        ...
        portfolio = self.exchange.get_portfolio()
        global entrytime
        if abs(portfolio["XBTUSD"]["currentQty"]) <= abs(portfolio["XBTZ19"]["currentQty"]):
            entrytime = time.time()

        else:
            entrytime = entrytime

        return float(time.time() - entrytime)

    def instrumentChoice(self):
    ...
    def get_price_offset(self, index):
        ...
        instrumentChoice = self.instrumentChoice()
        ticker = self.exchange.get_ticker(instrumentChoice)
        position = self.exchange.get_position(instrumentChoice)
        delta = self.exchange.calc_delta()['spot']
        currentQty = self.exchange.get_delta(instrumentChoice)
        portfolio = self.exchange.get_portfolio()
        expsosure = abs(portfolio["XBTUSD"]["currentQty"])

        if instrumentChoice == "XBTZ19":
            if currentQty < abs(portfolio["XBTUSD"]["currentQty"]):
                if currentQty == 0:
                    ...
                    if float(position['avgEntryPrice']) > ticker["sell"]:
                        ...
                        if currentQty <= abs(portfolio["XBTUSD"]["currentQty"]) - 50:
                            self.start_position_buy = ticker["buy"]
                            self.start_position_sell = ticker["sell"] + 42
                        else:
                            self.start_position_buy = ticker["buy"]
        ...
        if instrumentChoice == "XBTUSD":
            if currentQty == 0:
                ...
                if float(position['avgEntryPrice']) < ticker["buy"]:
                    ...
                    if abs(currentQty) <= abs(portfolio["XBTZ19"]["currentQty"]) - 50:
    ...
    def prepare_order(self, index):
        ...
        return {'price': price, 'orderQty': round(quantity), 'side': "Buy" if index < 0 else "Sell"}
    ...
    def converge_orders(self, buy_orders, sell_orders):
        ...
        tickLog = self.exchange.get_instrument(instrumentChoice)['tickLog']
        ...
        ###going to amend the logic for price placement first and see what happens! eek. exciting.




        existing_orders = self.exchange.get_orders(instrumentChoice)

        #it might not be getting existing orders.
        ...
        if len(to_amend) > 0:
            ...
            try:
                self.exchange.amend_bulk_orders(to_amend, symbol = instrumentChoice)
        ...
        if len(to_create) > 0:
            ...
            self.exchange.create_bulk_orders(to_create, symbol = instrumentChoice)
            buys_matched +=1
            sells_matched +=1
